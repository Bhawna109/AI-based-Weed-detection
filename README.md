...
### On a GPU (recommended) — Google Colab, free

Open **[notebooks/train_colab.ipynb](notebooks/train_colab.ipynb)** in
[Google Colab](https://colab.research.google.com/), set the runtime to a **T4
GPU**, and `Run all`. It clones this repo, downloads the dataset, trains
`yolo11s` for 100 epochs (~30–60 min), evaluates on the test split, and gives
you `best.pt` + metrics to download. ~50–100× faster than CPU.
...
python src/train.py            # add --device 0 if you have an NVIDIA GPU
```

> **Keep `dataset/` on plain local disk, not in OneDrive/Dropbox/Google Drive.**
> Cloud-sync folders make every epoch re-read images through the sync layer
> (2–3× slower training). `fetch_hf_dataset.py` writes to `dataset/` by default;
> move it elsewhere and set `path:` in `configs/data.yaml` (this repo's copy
> points at `C:/weed_data`).
...
├── dataset/                 # NOT committed - images live outside the repo (see dataset/README.md)
│   └── README.md
├── notebooks/
│   └── train_colab.ipynb    # one-click GPU training on Google Colab
├── docs/
│   ├── ARCHITECTURE.md      # YOLO11: input / backbone / neck / head / anchor-free
│   ├── ERROR_ANALYSIS.md    # failure-mode analysis + limitations (real findings)
